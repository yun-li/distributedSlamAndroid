...
#ifndef LOGI
#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO, "KFusionOMP", __VA_ARGS__))
#define LOGW(...) ((void)__android_log_print(ANDROID_LOG_WARN, "KFusionOMP", __VA_ARGS__))
#endif
#else
...
extern "C"
{
  JNIEXPORT jboolean JNICALL
    Java_project_pamela_slambench_jni_KFusion_kfusionompinit (JNIEnv * env,
							      jobject thiz,
							      jobjectArray
							      filename);
  JNIEXPORT jstring JNICALL
    Java_project_pamela_slambench_jni_KFusion_kfusionompprocess (JNIEnv *
								 env);
  JNIEXPORT jboolean JNICALL
    Java_project_pamela_slambench_jni_KFusion_kfusionomprelease (JNIEnv *
								 env);

  JNIEXPORT void JNICALL
    Java_project_pamela_slambench_jni_KFusion_kfusionompglinit (JNIEnv *);
  JNIEXPORT void JNICALL
    Java_project_pamela_slambench_jni_KFusion_kfusionompglresize (JNIEnv *,
								  jint width,
								  jint
								  height);
  JNIEXPORT void JNICALL
    Java_project_pamela_slambench_jni_KFusion_kfusionompglrender (JNIEnv *);

              JNIEXPORT jstring JNICALL
                Java_project_pamela_slambench_jni_KFusion_kfusionompinfo (JNIEnv *
            								 env);
...
JNIEXPORT jstring JNICALL
Java_project_pamela_slambench_jni_KFusion_kfusionompinfo (JNIEnv * env)
{

      std::ostringstream o;
      o << "Start infos" << std::endl;
      o << "omp_get_max_threads = " <<        omp_get_max_threads()   << std::endl;
      o << "omp_get_num_procs = " <<        omp_get_num_procs()   << std::endl;
...
JNIEXPORT jboolean JNICALL
Java_project_pamela_slambench_jni_KFusion_kfusionomprelease (JNIEnv * env)
{

  LOGI ("Start omp mem release");

  LOGI ("Free reader");
  if (reader)
    delete (reader);
    reader = NULL;
  LOGI ("Free kfusion object");
  if (kfusion)
    delete (kfusion);
    kfusion = NULL;
  LOGI ("Free buffers");
  ...
  LOGI ("Finish omp mem release");
  return true;

}

JNIEXPORT jstring JNICALL
Java_project_pamela_slambench_jni_KFusion_kfusionompprocess (JNIEnv * env)
{
